...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
	CREATE USER fcculs;
      ALTER USER fcculs PASSWORD '%My-S3cr3t%';

	CREATE DATABASE amat;
	GRANT ALL PRIVILEGES ON DATABASE amat TO fcculs;
	
	CREATE DATABASE gmrs;
	GRANT ALL PRIVILEGES ON DATABASE gmrs TO fcculs;
	EOSQL

psql -v ON_ERROR_STOP=1 --username fcculs --dbname amat <<-EOSQL
	CREATE TABLE AM
(
      ...
      PRIMARY KEY (record_type, unique_system_identifier),
      CONSTRAINT am_unique UNIQUE (record_type, unique_system_identifier)
);
...
(
      ...
      status_date              char(9)	           null,
      lic_category_code         char(1)	           null,
      linked_license_id         char(9)              null,
      linked_callsign           char(10)             null,
      
      PRIMARY KEY (record_type, unique_system_identifier),
      CONSTRAINT en_unique UNIQUE (record_type, unique_system_identifier)
);
...
(
      ...
      PRIMARY KEY (record_type, unique_system_identifier),
      CONSTRAINT hd_unique UNIQUE (record_type, unique_system_identifier)
);
...
(
      ...
      code                      char(6)              null,
      PRIMARY KEY (record_type, unique_system_identifier),
      CONSTRAINT hs_unique UNIQUE (record_type, unique_system_identifier)
);

EOSQL

psql -v ON_ERROR_STOP=1 --username fcculs --dbname gmrs <<-EOSQL
CREATE TABLE EN
(
      record_type               char(2)              not null,
      unique_system_identifier  numeric(9,0)         not null,
